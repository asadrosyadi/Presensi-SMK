...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $attendanceLateToday = Attendance::where('date', Carbon::now()->format('Y-m-d'))
            ...
            ->count();
        $attendanceLateToday2 = Attendance::where('date', Carbon::now()->format('Y-m-d'))
            ->where('out_time', '>', $getSetting->out_time)
            ->count();    
        $areaCount = Area::count();

        return view('home', compact('userCount', 'attendaceToday', 'attendanceLateToday', 'attendanceLateToday2','areaCount'));
    }
}
